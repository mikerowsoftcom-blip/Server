...
#
# IMPORTANT: llama-server now bundles multimodal (mtmd) support, which pulls
# in a large number of vision-model backends and makes the build very
# memory-hungry. Render's build machines have a fixed RAM budget regardless
# of core count, so building with -j"$(nproc)" (one job per core) reliably
# OOMs there ("Ran out of memory (used over 8GB) while building"). Capping
# parallelism keeps peak memory down; it just makes the build slower.
RUN cmake -B build \
