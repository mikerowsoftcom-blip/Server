...
- `GGML_NATIVE=ON` — compiles for the exact CPU ISA of the build machine
  (AVX2/AVX-512/FMA/etc.) instead of a generic portable target.
- `GGML_OPENBLAS=ON` — BLAS-accelerated matmul, which mainly speeds up
  prompt/prefill processing (long system prompts, RAG context).
- `GGML_LTO=ON` — link-time optimization across the build.
